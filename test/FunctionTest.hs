...
testFunctionDefinitionParserBooleanType = assertEqual "parseFunctionDefBooleanType" (parse functionDefinitionParser "" "boolean my_function() {true != false} return true  end") (Right (FuncDefinition BooleanType (Ident "my_function") (Parameters []) (Block [Expression (BooleanExpression (BooleanOp (Literal (BooleanLiteral True)) NotEqual (Literal (BooleanLiteral False))))]) (Right (Literal (BooleanLiteral True)))))
...
testFunctionDefinitionWithWrongFunctionType = assertEqual "parseFunctionDefWithWrongTypeFunction" (parse functionDefinitionParser "" "int my_function(int a, int b, boolean c) {a <= b} return true end") (Right (FuncDefinition IntType (Ident "my_function") (Parameters [(IntType, Ident "a"), (IntType, Ident "b"), (BooleanType, Ident "c")]) (Block [Expression (BooleanExpression (BooleanOp (Identifier (Ident "a")) LessEqualThan (Identifier (Ident "b"))))]) (Left (ErrorType TypeFunction))))
